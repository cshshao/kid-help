<template>
  <el-aside class="index-aside" height="100vh" width="180px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item" style="height:100%;broder:0;background-color:#E31396">
        <div class="menulistImg" style="backgroundColor:#C71585;padding:25px 0" v-if="false && menulistStyle == 'vertical'">
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" style="height:100%;" background-color="#E31396" text-color="#EEEEF4" active-text-color="#F2DD3D" default-active="0">
          ...
          <el-submenu :index="1+''" :style="menulistBorderBottom">
            <template slot="title">
              ...
              <span style="color: #4683fb">个人中心</span>
            </template>
            <el-menu-item :index="1-1" style="color: #1a1a1a" @click="menuHandler('updatePassword')">修改密码</el-menu-item>
            <el-menu-item :index="1-2" style="color: #1a1a1a" @click="menuHandler('center')">个人信息</el-menu-item>
          </el-submenu>
          <el-submenu :style="menulistBorderBottom" v-for=" (menu,index) in item.backMenu" :key="menu.menu" :index="index+2+''">
            <template slot="title">
              ...
              <span style="color: #1b1e21">{{ menu.menu }}</span>
            </template>
      ...
      this.$nextTick(()=>{
        document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "rgba(248, 248, 248, 1)"
          })
          el.addEventListener("mouseleave", e => {
            e.stopPropagation()
            el.style.backgroundColor = "#E31396"
          })
